...
public class Cliente {
    ...
    public ResultSet listarClientesPorTipo(String tipo) throws Exception {
        ResultSet rs = null;
        strSQL = "select num_doc as \"Num. Identidad\",apellidos||', '||nombres as \"Nombre completo\","
                + "tipo,telefono from cliente where tipo='"+tipo+"' and estado=true;";
        try {
            rs = objConectar.consultarBD(strSQL);
            return rs;
        } catch (Exception e) {
            throw new Exception("Error al listar clientes por tipo: " + e.getMessage());
        }
    }
    
    public int obtenerIdPorDNI(String dni) throws Exception {
        ResultSet rs = null;
        strSQL = "select * from cliente where num_doc='"+dni+"'";
        try {
            rs = objConectar.consultarBD(strSQL);
            while (rs.next()) {                
            ...
            throw new Exception("Error al obtner id de cliente por dni " + e.getMessage());
        }
        return 0;
    }
}
